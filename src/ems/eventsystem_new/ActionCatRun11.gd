...
func _on_event_started():
	teleport_mesh = get_parent().get_parent().teleporters_container.get_node("2Back2_jpg")
	cat.rotation.y = atan2(4.156, -1.02)
...
func _on_cat_on_screen_notif_screen_entered():
	if ensure_journal_read:
		...
		tween.tween_property(cat, "position", Vector3(4.156, -1.398, -1.02), 2).set_ease(Tween.EASE_IN_OUT).set_trans(Tween.TRANS_LINEAR)
...
func _on_journal_hermano_pule_exhibit_secondary_emit():
