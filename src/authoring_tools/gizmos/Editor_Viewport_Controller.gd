...
@export var path_to_texture: NodePath

# A exported NodePath to the Editor_Camera_Controller node.
@export var path_to_editor_camera: NodePath
...
func _ready():
	...
	editor_camera.physics_object_selected.connect(physics_object_selected);
	...
	editor_controller.on_editor_mode_change.connect(on_editor_mode_change);
...
func send_editor_raycast():
	...
	var params_gizmo = PhysicsRayQueryParameters3D.new()
	...
	params_gizmo.from = gizmo_camera.project_ray_origin(get_mouse_position())
	params_gizmo.to = params_gizmo.from + gizmo_camera.project_ray_normal(get_mouse_position()) * 100;
	params_gizmo.exclude = [self]
	params_gizmo.collision_mask = GIZMO_COLLISION_LAYER
	...
	var result = space_state.intersect_ray(params_gizmo);
