...
func _physics_process(delta):
	...
	if (send_raycast == true):
		...
		# UPGRADED TO GODOT 4 FROM GODOT 3 
		...
		params.from = view_camera.project_ray_origin(get_tree().root.get_mouse_position())
		params.to = params.from + view_camera.project_ray_normal(get_tree().root.get_mouse_position()) * 100;
		params.exclude = [self]
		params.collision_mask = NORMAL_COLLISION_LAYER
		...
		physics_object_selected.emit(selected_node)
